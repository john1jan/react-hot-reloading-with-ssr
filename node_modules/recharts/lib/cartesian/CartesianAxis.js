...
var CartesianAxis = (_temp = _class = function (_Component) {
  ...
  _createClass(CartesianAxis, [{
    ...
    value: function getTicks(props) {
      ...
      return (0, _DataUtils.isNumber)(interval) || (0, _ReactUtils.isSsr)() ? CartesianAxis.getNumberIntervalTicks(ticks, (0, _DataUtils.isNumber)(interval) ? interval : 0) : CartesianAxis.getAutoIntervalTicks(ticks, tickFormatter, viewBox, orientation, minTickGap);
    }
  }, {
    ...
    value: function getAutoIntervalTicks(ticks, tickFormatter, viewBox, orientation, minTickGap) {
      ...
      var step = Math.ceil(len / 2);

      for (var i = 0; i < step; i++) {
        // the middle tick
        if (len % 2 && i === len - 1 - i) {
          var middle = result[i];
          var content = (0, _isFunction3.default)(tickFormatter) ? tickFormatter(middle.value) : middle.value;
          var size = (0, _DOMUtils.getStringSize)(content)[sizeKey];
          var isShow = sign * (middle.coordinate - sign * size / 2 - start) >= 0 && sign * (middle.coordinate + sign * size / 2 - end) <= 0;

          if (isShow) {
            result[i] = _extends({}, result[i], { tickCoord: middle.coordinate, isShow: isShow });
          }
        } else {
          var head = result[i];
          var headContent = (0, _isFunction3.default)(tickFormatter) ? tickFormatter(head.value) : head.value;
          var headSize = (0, _DOMUtils.getStringSize)(headContent)[sizeKey];
          var tail = result[len - 1 - i];
          ...
          if (i === 0) {
            ...
            result[len - 1 - i] = tail = _extends({}, tail, {
              tickCoord: tailGap > 0 ? tail.coordinate - tailGap * sign : tail.coordinate
            });

            var headGap = sign * (head.coordinate - sign * headSize / 2 - start);
            result[i] = head = _extends({}, head, {
              tickCoord: headGap < 0 ? head.coordinate - headGap * sign : head.coordinate
            });
          } else {
            result[i] = head = _extends({}, head, { tickCoord: head.coordinate });
            result[len - 1 - i] = tail = _extends({}, tail, { tickCoord: tail.coordinate });
          }
          ...
          if (isTailShow) {
            ...
            result[len - 1 - i] = _extends({}, tail, { isShow: true });
          }

          var isHeadShow = sign * (head.tickCoord - sign * headSize / 2 - start) >= 0 && sign * (head.tickCoord + sign * headSize / 2 - end) <= 0;

          if (isHeadShow) {
            start = head.tickCoord + sign * (headSize / 2 + minTickGap);
            result[i] = _extends({}, head, { isShow: true });
          }
        }
      }
  ...
  ticks: _react.PropTypes.oneOfType([_react.PropTypes.arrayOf(_react.PropTypes.shape({
